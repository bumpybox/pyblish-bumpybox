...
class BumpyboxMayaModelingRepairIntermediateShapes(api.Action):
    label = "Repair"
    icon = "wrench"
    on = "failed"
...
class BumpyboxMayaModelingValidateIntermediateShapes(api.InstancePlugin):
    """ Ensures there are no intermediate shapes in the scene. """

    families = ["mayaAscii", "mayaBinary", "alembic"]
    label = "Intermediate Shapes"
    order = api.ValidatorOrder
    actions = [BumpyboxMayaModelingRepairIntermediateShapes]
    optional = True
