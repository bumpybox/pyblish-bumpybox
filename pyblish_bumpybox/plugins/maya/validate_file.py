...
class BumpyboxMayaRepairFilePath(pyblish.api.Action):
    ...
    def process(self, context, plugin):
        ...
        for instance in instances:
            ...
            expected = BumpyboxMayaValidateFilePath().get_expected_path(
                instance
            )
...
class BumpyboxMayaValidateFilePath(pyblish.api.InstancePlugin):
    """ Validate file paths to be in the workspace directory. """

    order = pyblish.api.ValidatorOrder
    families = ["file"]
    label = "File Path"
    actions = [BumpyboxMayaRepairFilePath]
